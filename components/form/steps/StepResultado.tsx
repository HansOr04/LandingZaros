...
export const StepResultado = () => {
    ...
    const { nombre, fechaReunion, horaReunion } = data;

    const nombrePrint = nombre?.split(' ')[0] || 'tú';

    // WhatsApp CTA building
    const programaRef = rec.titulo;
    const fechaRef = fechaReunion ? formatFecha(new Date(fechaReunion)) : '';
    const msg = `Hola, mi nombre es ${nombrePrint}. Acabo de completar mi postulación a Zaros Latam y me recomendaron el ${programaRef}.${fechaRef ? ` Agendé mi reunión para el ${fechaRef} a las ${horaReunion}.` : ''}`;
    ...
    return (
        <div className="w-full flex flex-col gap-[32px] pb-10">
            {/* 1. Hero */}
            <motion.div
                custom={0}
                initial="hidden"
            ...
            >
                ...
                <div className="relative z-[1] flex flex-col items-center">
                    ...
                    <p className="text-[0.87rem] text-white/70 leading-[1.6] max-w-[280px]">
                        Basándonos en tu perfil y experiencia, nuestra IA identificó el nivel del programa Zaros que mejor potenciará tu liderazgo actual.
                    </p>
                </div>
            </motion.div>
            ...
            {fechaReunion && horaReunion && (
                ...
                >
                    <div className="bg-[#FAF7F2] border border-[#EBE3F0] rounded-[16px] p-[18px_22px] flex items-center gap-[14px]">
                        <div className="text-[1.9rem] flex-shrink-0">📅</div>
                        <div className="flex flex-col gap-1 w-full">
                            <span className="font-bold text-[#1E0E35] text-[0.95rem]">
                                {formatFecha(new Date(fechaReunion))} a las {horaReunion}
                            </span>
                            <span className="text-[#5A4870] text-[0.8rem]">Entrevista · 30 min · Google Meet</span>
                    ...
                    <div className="bg-[#C8447A]/5 border border-[#C8447A]/20 rounded-[14px] p-[16px_20px] mt-[12px]">
                        <p className="text-[0.88rem] text-[#3B1260] leading-[1.6]">
                            🌟 <strong>¡Estás a un paso!</strong> Nuestro equipo se comunicará contigo el <strong>{formatFechaCorta(new Date(fechaReunion))}</strong> a las <strong>{horaReunion}</strong> para tu entrevista de admisión. Revisa tu correo — ya tienes tu invitación.
                        </p>
                    </div>
                </motion.div>
            )}
            ...
            >
                ...
                <div className="p-[26px]">
                    <h3 className="font-serif text-[1.25rem] font-bold text-[#3B1260] mb-5 leading-snug">
                        {rec.titulo}
                    </h3>
                    <blockquote className="bg-[#FAF7F2] border-l-[3px] border-[#C8447A] rounded-r-[12px] p-[15px_17px] text-[0.86rem] text-[#5A4870] italic leading-[1.65] mb-6">
                        {rec.razon}
                    </blockquote>
            ...
            {rec.otros.length > 0 && (
                ...
                >
                    <span className="text-[0.68rem] text-[#9B8EB0] font-bold uppercase tracking-wider pl-1">
                        También podrías explorar
                    </span>
                    <div className="grid grid-cols-2 gap-3">
                        {rec.otros.map(n => {
                            const labels: any = {
                                nivel1: { tag: 'Nivel 1', t: 'Empoderamiento' },
                                nivel2: { tag: 'Nivel 2', t: 'Marca Personal' },
                                nivel3: { tag: 'Nivel 3', t: 'Lid. Multiplicador' },
                            };
                            return (
                                <div key={n} className="bg-white border-[1.5px] border-[#EBE3F0] rounded-[15px] p-[17px] cursor-pointer hover:border-[#6C3BA5]/40 hover:shadow-md transition-all">
                                    <div className="text-[0.68rem] font-bold text-[#9B8EB0] mb-1">{labels[n].tag}</div>
                                    <div className="text-[0.87rem] font-bold text-[#3B1260]">{labels[n].t}</div>
                                </div>
                            );
                        })}
                    </div>
            ...
            <motion.div
                ...
                animate="visible"
                variants={sectionVariants}
                className="flex flex-col gap-[11px] mt-2"
            >
                <a
                    href={waLink}
                    target="_blank"
                ...
                >
                    ...
                    Confirmar entrevista de admisión por WhatsApp
                </a>

                <div className="flex items-center justify-center gap-5 flex-wrap mt-4 text-[0.85rem] text-[#6C3BA5] font-semibold">
